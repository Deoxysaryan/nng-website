...
- Clean live measurement (24 Sept 2026, preview commit `1740562`, the Mac quiet enough, three runs per page): Performance 100 on all five pages once GitHub's servers had the new files cached (LCP 1.4 to 1.7 s, total blocking time 10 to 30 ms, CLS 0); Accessibility and Best Practices 100 in every run. The only runs below 100 (Services 97, About 96, Home 99) were the first requests after a deploy, when GitHub's server latency was 30 to 280 ms instead of about 10 ms. Earlier readings of 75 to 98 came from a busy machine, not the site. Measure twice after any deploy and use the second run.
